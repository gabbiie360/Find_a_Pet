<template>
  <section class="adoption-section">
    <div class="container">
      ...
      <div v-else-if="pets.length === 0 && !error" class="empty-msg">No hay mascotas en adopción en este momento.</div>
      ...
      <div v-else class="pets-grid">
        <div v-for="mascota in pets" :key="mascota._id" class="pet-card" v-fade-in-on-scroll>
          <img :src="mascota.fotos[0] || placeholderImage" alt="Mascota en adopción" class="pet-image" />
          <div class="pet-info">
            <h3>{{ mascota.nombre }}</h3>
            <p>{{ mascota.especie }} | {{ mascota.ciudad }}</p>
            <router-link :to="`/pet/${mascota._id}`" class="btn-more">Conóceme</router-link>
          </div>
        </div>
      </div>
    </div>
...
onMounted(async () => {
  try {
    // Usamos la función correcta con el filtro adecuado
    const response = await ReportService.getAllPublicReports({ estado: 'adopcion' });
    pets.value = response.data;
  } catch (err) {
    console.error('Error al obtener mascotas en adopción:', err);
    error.value = 'No se pudieron cargar las mascotas en adopción.';
  } finally {
    loading.value = false;
  }
});
...
.adoption-section { background-color: #fcf7ff; padding: 4rem 1rem; text-align: center; }
...
.pets-grid { display: grid; grid-template-columns: repeat(auto-fill, minmax(250px, 1fr)); gap: 2rem; margin-top: 2rem; }
.pet-card { background: #fff; border-radius: 15px; box-shadow: 0 5px 20px rgba(0, 0, 0, 0.05); overflow: hidden; transition: 0.3s ease; }
.pet-card:hover { transform: translateY(-5px); }
.pet-image { width: 100%; height: 180px; object-fit: cover; }
.pet-info { padding: 1rem; }
.pet-info h3 { margin: 0; font-size: 1.3rem; color: #333; }
.pet-info p { margin: 0.3rem 0; color: #777; }
